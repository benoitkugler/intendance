...
CREATE TABLE groupes (
	...
	CREATE OR REPLACE FUNCTION structgen_validate_json_struct_1 (data jsonb)
		RETURNS boolean
		AS $f$
	BEGIN
	...
	CREATE OR REPLACE FUNCTION structgen_validate_json_Unite (data jsonb)
		RETURNS boolean
		AS $f$
	BEGIN
		RETURN jsonb_typeof(data) = 'string' AND data#>>'{}' IN ('Kg', 'L', 'P', '');
	END;
	$f$
	LANGUAGE 'plpgsql'
	IMMUTABLE;

	CREATE OR REPLACE FUNCTION structgen_validate_json_struct_2143226399 (data jsonb)
		RETURNS boolean
		AS $f$
	BEGIN
		IF jsonb_typeof(data) != 'object' THEN 
		...
		END IF;
		RETURN (SELECT bool_and( 
			key IN ('quantite', 'unite')
		) FROM jsonb_each(data))  
		AND structgen_validate_json_number(data->'quantite')
AND structgen_validate_json_Unite(data->'unite')
		;
	END;
	$f$
	LANGUAGE 'plpgsql'
...
CREATE TABLE ingredients (
	id serial PRIMARY KEY,
	nom varchar  NOT NULL,
	unite varchar  CHECK (unite IN ('Kg', 'L', 'P', '')) NOT NULL,
	categorie varchar  NOT NULL,
	callories jsonb  NOT NULL CONSTRAINT callories_structgen_validate_json_struct_1 CHECK (structgen_validate_json_struct_1(callories)),
	conditionnement jsonb  NOT NULL CONSTRAINT conditionnement_structgen_validate_json_struct_2143226399 CHECK (structgen_validate_json_struct_2143226399(conditionnement))
);
...
CREATE TABLE livraisons (
	...
	id_fournisseur integer  NOT NULL,
	nom varchar  NOT NULL,
	jours_livraison boolean[]  CHECK (array_length(jours_livraison, 1) = 7) NOT NULL,
...
CREATE TABLE repass (
	...
	horaire integer  CHECK (horaire IN (4, 3, 2, 1, 0)) NOT NULL,
	anticipation integer  NOT NULL
);
